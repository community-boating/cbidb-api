...
case class EntityVisibility(
	...
	instanceFilter: Option[(RequestCache[_] => Set[Int])], // Function that specifies which IDs the user can pull.
...
object EntityVisibility {
	...
	val ZERO_VISIBILITY = EntityVisibility(entityVisible = false, Some((rc: RequestCache[_]) => Set.empty), Some(Set.empty))
