...
class DonationFilterFactoryDateRange extends ReportingFilterFactory[Donation] {
	override val displayName: String = "Within Date Range"
	override val argDefinitions = List(
		(ARG_DATE, DateLogic.now.toLocalDate.format(DateTimeFormatter.ofPattern("MM/dd/yyyy"))),
		(ARG_DATE, DateLogic.now.toLocalDate.format(DateTimeFormatter.ofPattern("MM/dd/yyyy")))
	)

	override def getFilter(rc: UnlockedRequestCache, arg: String): ReportingFilter[Donation] = new ReportingFilterFunction(rc, (rc: UnlockedRequestCache) => {
		val split = arg.split(",")
