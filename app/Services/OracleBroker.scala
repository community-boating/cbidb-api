...
class OracleBroker @Inject() (lifecycle: ApplicationLifecycle, cp: ConnectionPoolConstructor) extends PersistenceBroker {
  OracleBroker.createPool(cp, () => {
    lifecycle.addStopHook(() => Future.successful({
      println("  ************    Shutting down!  Closing pool!!  *************  ")
      OracleBroker.closePool
    }))
  })



  val pool = OracleBroker.getPool
