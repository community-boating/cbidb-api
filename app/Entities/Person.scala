package Entities

import CbiUtil.{SelfInitializable}
import Entities.PersonRating.CasePersonRating
import Services.{PersistenceBroker, RequestCache}
import Storable.Fields.FieldValue.IntFieldValue
import Storable.Fields.IntDatabaseField
import Storable._

class Person(implicit pb: PersistenceBroker, rc: RequestCache) extends StorableClass {
  override val self: Person = this
  this.setCompanion(Person)
  object references extends ReferencesObject {
    var personRatings = new SelfInitializable[Set[PersonRating]](() => pb.getObjectsByFilters(
      PersonRating,
      List(PersonRating.fields.personId.equalsConstant(self.values.personId.get))
    ).toSet)
  }
  object values extends ValuesObject {
    val personId = new IntFieldValue(self, Person.fields.personId)
  }

  lazy val caseRatings: Set[CasePersonRating] = references.personRatings.get.map(_.asCaseClass)

  def hasRatingDirect(ratingId: Int, programId: Int): Boolean = caseRatings.contains(CasePersonRating(
    this.values.personId.get,
    ratingId,
    programId
  ))

  def hasRatingSomehow(ratingId: Int, programId: Int): Boolean = {
    if (hasRatingDirect(ratingId, programId)) true
    else {
      val subRatings: Set[Rating] = rc.ratings.get.filter(_.values.overriddenBy.get == Some(ratingId))
      subRatings.map(_.values.ratingId.get).foldLeft(false)((agg, r) => agg || hasRatingSomehow(r, programId))
    }
  }
}

object Person extends StorableObject[Person] {
  val entityName: String = "PERSONS"

  object fields extends FieldsObject {
    val personId = new IntDatabaseField(self, "PERSON_ID")
  }

  def primaryKey: IntDatabaseField = fields.personId
}