...
abstract class StorableObject[T <: StorableClass](implicit manifest: scala.reflect.Manifest[T], persistenceSystem: PersistenceSystem) {
	...
	def construct(ps: ProtoStorable, tableAlias: Option[String]): T = {
		val embryo: T = manifest.runtimeClass.newInstance.asInstanceOf[T]

		type FieldDefinition = (String, DatabaseField[_])

		def aliasField[U <: DatabaseField[_]](f: U): ColumnAliasInnerJoined[_] = tableAlias match {
			case Some(a) => ColumnAliasInnerJoined[U](TableAliasInnerJoined(self, a), f).asInstanceOf[ColumnAliasInnerJoined[_]]
